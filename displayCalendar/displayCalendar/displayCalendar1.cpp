...
int main() {
    ...
    for (int curMonth = 0; curMonth < 12; curMonth++) {
        ...
        for (string currentDay : DAYS_OF_WEEK) {
            ...
            cout << currentDay << " "; //this prints out each dayofweek as the loop iterates
       ...
       // std::printf("%d %s\n", __LINE__, "print blank boxes before first day");
        ...
        for (int column = 0; column < startDay; column++) {
            ...
            // to be completed by student
            cout << "    "; //this creates blank placeholders for days that are not apart of the specific month at the beginning

        }

       //std::printf("%d %s\n", __LINE__, "print dates on each row"); I think these are just directions and not important to the calendar display
        //Print days
        dayCounter = 1;
        while (dayCounter <= daysInMonth[curMonth]) {
            ...
            for (int column = startDay; column < 7 && dayCounter <= daysInMonth[curMonth]; column++) {
                ...
                // to be completed by student
               std::printf("%-3d ", dayCounter);

                dayCounter++; //this is the specific number day of the month
                startDay++;
            }
            std::printf("\n");
            startDay %= 7; // startDay is day of the week
        }
